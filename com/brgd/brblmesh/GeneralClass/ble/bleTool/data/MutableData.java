...
public class MutableData extends Data {
    ...
    private static int sfloatToInt(float f) {
        ...
        double dAbs = Math.abs(((double) Math.round(fAbs * 10000.0f)) - ((double) (Math.round(fAbs) * SFLOAT_PRECISION)));
        while (dAbs > 0.5d && i2 > SFLOAT_EXPONENT_MIN) {
            float f2 = fAbs * 10.0f;
            if (f2 > 2045.0f) {
                break;
            }
            i2--;
            dAbs = Math.abs(((double) Math.round(f2 * 10000.0f)) - ((double) (Math.round(f2) * SFLOAT_PRECISION)));
            fAbs = f2;
        }
    ...
    private static int floatToInt(float f) {
        ...
        double dAbs = Math.abs(((double) Math.round(fAbs * 1.0E7f)) - ((double) (Math.round(fAbs) * FLOAT_PRECISION)));
        while (dAbs > 0.5d && i2 > FLOAT_EXPONENT_MIN) {
            ...
            i2--;
            dAbs = Math.abs(((double) Math.round(f2 * 1.0E7f)) - ((double) (Math.round(f2) * FLOAT_PRECISION)));
            fAbs = f2;
        }
        return (Math.round(i * fAbs) & ViewCompat.MEASURED_SIZE_MASK) | (i2 << 24);
    }
}
