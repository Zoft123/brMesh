    ...
    public boolean executeKeyEvent(KeyEvent keyEvent) {
        ...
        if (!zCanScroll) {
            if (isFocused() && keyEvent.getKeyCode() != 4) {
                ...
                View viewFindNextFocus = FocusFinder.getInstance().findNextFocus(this, viewFindFocus, GattError.GATT_WRONG_STATE);
                if (viewFindNextFocus != null && viewFindNextFocus != this && viewFindNextFocus.requestFocus(GattError.GATT_WRONG_STATE)) {
                    return true;
                }
            }
            return false;
        ...
        if (keyEvent.getAction() == 0) {
            ...
            if (keyCode == 20) {
                if (!keyEvent.isAltPressed()) {
                    return arrowScroll(GattError.GATT_WRONG_STATE);
                }
                return fullScroll(GattError.GATT_WRONG_STATE);
            }
            if (keyCode == 62) {
